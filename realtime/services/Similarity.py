...
def find_similar_emails(new_email_text, top_k=5):
    ...
    try:
     ...
     print("[INFO] Loaded existing email embeddings and texts.")

    except FileNotFoundError:
     print("[INFO] Computing embeddings for the first time...")
    ...
    # ✅ Save embeddings & texts
     ...
     print("[OK] Saved email embeddings & texts.")
...
# ✅ Load FAISS index (if exists) or create a new one
    try:
     ...
     print("[OK] Created and saved FAISS index.")
    
      
    if index.ntotal == 0:
        print("[WARN] FAISS index is empty! No data to search.")
        return []
    ...
    if not results:
        print("[WARN] No similar emails found.")
